...
const footerStopTrigger = ScrollTrigger.create({
    ...
    onEnter: () => {
        ...
        scrollPaused = true;
        lenis.stop();
        setTimeout(() => { lenis.start(); scrollPaused = false; }, 500);
    },
    ...
    filterItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            cards.forEach(card => {
                ...
                if (filterText === 'all') {
                    isMatch = true;
                } else if (filterText === 'ui/ux & web design') {
                ...
                } else if (filterText === 'graphic design') {
                    isMatch = cardTags.some(tag => tag.includes('graphic'));
                } else if (filterText === 'mobile') {
                    isMatch = cardTags.some(tag => tag.includes('mobile'));
                } else if (filterText === 'ai') {
                    isMatch = cardTags.some(tag =>
                        tag === 'ai' || tag.startsWith('ai ') || tag.includes(' ai ')
                    );
                }
...
const material = new THREE.PointsMaterial({
    color: 0xf97316, size: 0.158, transparent: true, opacity: 0.85,
